...
class heartbeat_program_ipc(object):
    def __init__(self):
    # get the hostname
        ...
        self.pub_sentence = rospy.Publisher("/heartbeat/sentence",String,queue_size = 1)
    ...
    def cb_heartbeat(self,event):
        ...
        #print("fuck")
        print(self.data)
        self.pub_sentence.publish(self.data)
    ...
    def entrance_task(self):
        sentence = "RXGAT," +self.day+self.month+self.year+","+self.hour +self.minute+self.second+","+ self.teamid+","+str(self.entrance)+ ","+str(self.exit)
        check = self.checksum(sentence)
...
if __name__ == '__main__':
    rospy.init_node("heartbeat_program_ipc")
    Heartbeat_program_ipc = heartbeat_program_ipc()
    rospy.spin()