...
class Receiver(object):
    def __init__(self):
        ...
        self.num_veh = rospy.get_param('~num_veh',1)
        self.ser = serial.Serial(self.port, self.baud, timeout=3)

        if self.log:
            self.pub_log = rospy.Publisher('/lora_log',GloraPack,queue_size=1)
        ...
        # Publications
        #self.pub_husky_pose = rospy.Publisher("/husky/pose_new", PoseStamped, queue_size=1)
        #self.pub_husky_mode = rospy.Publisher("/husky/mode", Bool, queue_size=1)
        #self.pub_duckiefloat_point= rospy.Publisher("/duckiefloat/glora_points", Float32MultiArray, queue_size=1)
        
        self.pub_artipose_list = rospy.Publisher("/artifacts_pose_list",ArtifactPoseArray,queue_size=1)
            ...
            try:
                ...
                for i in range(len(idbytes_list)):
                    ...
                    if idbytes_list[i] & 0x0f == 0x00:
                        ...
                        if idx_veh == 1:
                            #self.pub_husky_pose.publish(msgs_list[i])
                            mode_msg = Bool()
                            mode_msg.data = True if data_list[i] == 1 else False
                            #self.pub_husky_mode.publish(mode_msg)
                            if data_list[i] == 1:
                                rospy.logerr('husky is waiting further instruction')
                            #rospy.loginfo(hex(idbytes_list[i]) + ': Husky pose published.')
                    ...
                    #
                        ...
                        if idbytes_list[i] & 0x0f == 0x0A:                    # Extinguisher
                            ...
                            artipose.Class = 'Fire Extinguisher'
                            artipose.appear_count = idx_veh
                            artipose.probability = data_list[i]
                            artipose.pose = pose
                            arti_list.count += 1
                            arti_list.pose_array.append(artipose)
                            #rospy.loginfo(hex(idbytes_list[i]) + ': Extinguishers PoseArray published')

                        elif idbytes_list[i] & 0x0f == 0x0B:                  # Drill
                            artipose = ArtifactPose()
                            artipose.Class = 'Drill'
                            artipose.appear_count = idx_veh
                            artipose.probability = data_list[i]
                            artipose.pose = pose
                            arti_list.count += 1
                            arti_list.pose_array.append(artipose)
                            #rospy.loginfo(hex(idbytes_list[i]) + ': Drills PoseArray published')
                            ...
                            arti_list.count += 1
                            arti_list.pose_array.append(artipose)
                            #rospy.loginfo(hex(idbytes_list[i]) + ': Survivors PoseArray published')
                            ...
                            arti_list.count += 1
                            arti_list.pose_array.append(artipose)
                            #rospy.loginfo(hex(idbytes_list[i]) + ': Backpacks PoseArray published')

                        elif idbytes_list[i] & 0x0f == 0x0E:                  # Smartphone
                            artipose = ArtifactPose()
                            artipose.Class = 'Cell Phone'
                            artipose.appear_count = idx_veh
                            artipose.probability = data_list[i]
                            artipose.pose = pose
                            #arti_list.count += 1
                            #arti_list.pose_array.append(artipose)
                            #rospy.loginfo(hex(idbytes_list[i]) + ': Phones PoseArray published')
...
if __name__ == "__main__":
    rospy.init_node("lora_host", anonymous=False)
    recv_node = Receiver()
