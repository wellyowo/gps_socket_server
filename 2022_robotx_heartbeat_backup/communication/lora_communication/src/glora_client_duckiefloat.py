...
MAX_NUM_OF_PACKLIST = 10
ROBOT_POSE_SAMPLE_PERIOD = 2
...
class Sender(object):
    def __init__(self):
        ...
        self.sub_beam = rospy.Subscriber('/glora_point', Float32MultiArray,self.cb_beam,queue_size=1)
        self.beam = Float32MultiArray()
        ...
        self.timer_arrange = rospy.Timer(rospy.Duration(ROBOT_POSE_SAMPLE_PERIOD), self.arrange_packets)

    def cb_beam(self,msg):
        
        # duckiefloat 8 beam value:
        gpack = GloraPackage(self.identity_byte | 0x00, msg=copy.deepcopy(msg))
    ...
    def subt_msg_cb(self, msg):
        ...
        # duckiefloat 8 beam value:
        gpack = GloraPackage(self.identity_byte | 0x00, msg=copy.deepcopy(msg.???????),
                             time_stamp=copy.deepcopy(msg.header.stamp))
        ...
        for i in range(msg.artifacts.count):
            ...
            probability = msg.artifacts.pose_array[i].probability
            if msg.artifacts.pose_array[i].Class == 'bb_extinguisher':
                gpack = GloraPackage(self.identity_byte |
                                     0x0A, msg=pose_msg, time_stamp=stamp, data=probability)

            elif msg.artifacts.pose_array[i].Class == 'bb_drill':
                gpack = GloraPackage(self.identity_byte |
                                     0x0B, msg=pose_msg, time_stamp=stamp, data=probability)

            elif msg.artifacts.pose_array[i].Class == 'bb_man':
                gpack = GloraPackage(self.identity_byte |
                                     0x0C, msg=pose_msg, time_stamp=stamp, data=probability)

            elif msg.artifacts.pose_array[i].Class == 'bb_backpack':
                gpack = GloraPackage(self.identity_byte |
                                     0x0D, msg=pose_msg, time_stamp=stamp, data=probability)

            elif msg.artifacts.pose_array[i].Class == 'bb_phone':
                gpack = GloraPackage(self.identity_byte |
                                     0x0E, msg=pose_msg, time_stamp=stamp, data=probability)
    ...
    def arrange_packets(self,event):
...
if __name__ == "__main__":
    rospy.init_node("glora_clinet", anonymous=False)
    trans_node = Sender()
